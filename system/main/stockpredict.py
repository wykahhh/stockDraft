...
def get_csv(code):
    df = ts.get_k_data('sh600519','2016-06-01')
...
class Config:
    # 数据参数
    feature_columns = list(range(3, 6))     # 要作为feature的列，按原数据从0开始计算，也可以用list 如 [2,4,6,8] 设置
    label_columns = [3]                  # 要预测的列，按原数据从0开始计算, 如同时预测第四，五列 最低价和最高价
...
def draw(config: Config, origin_data: Data, logger, predict_norm_data: np.ndarray):
    ...
    for i in range(label_column_num):
        ...
        temp_result =  str(np.squeeze(predict_data[-config.predict_day:, i]))
    ...
    return temp_result,str(loss_norm)

def True_pred(config=Config()):
    logger = load_logger(config)
    true_pred_result = str()
    true_pred_mean_square_error = str()
    try:
        ...
        if config.do_predict:
            ...
            true_pred_result,true_pred_mean_square_error = draw(config, data_gainer, logger, pred_result)
    except Exception:
        logger.error("Run Error", exc_info=True)
    return true_pred_result,true_pred_mean_square_error
